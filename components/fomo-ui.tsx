...
function resolveCryptoIcon(id?: string, symbol?: string, iconUrl?: string): ImageSourcePropType | undefined {
  ...
  return getCoinBySlugOrSymbol(id ?? symbol)?.icon;
}
    ...
    <View style={styles.tradeCard}>
      <View style={styles.tradeTop}>
        <View style={styles.tradeAvatar}>
          <Text style={styles.tradeAvatarText}>{avatar}</Text>
        </View>
...
export function PriceDelta({ value, positive }: { value: string; positive: boolean }) {
  return (
    <Text style={[styles.delta, positive ? styles.deltaPos : styles.deltaNeg]}>{positive ? '▲ ' : '▼ '}{value}</Text>
  );
}
    ...
    <Pressable disabled={!onPress} onPress={onPress} style={styles.tokenRow}>
      <View style={styles.tokenLeft}>
        <View style={styles.tokenIcon}>
          {iconSource ? (
            <Image source={iconSource} style={styles.tokenImage} resizeMode="contain" />
          ) : (
            <Text style={styles.tokenIconText}>{token.icon ?? token.symbol.slice(0, 1)}</Text>
          )}
        </View>
        <View>
          ...
          <View style={styles.subline}>
            ...
            {token.badges ? (
              <View style={styles.badges}>
                {token.badges.map((badge: string) => (
                  <View style={styles.badgeBubble} key={`${token.id}-${badge}`}>
        ...
        <View style={[styles.tokenLeft, styles.searchLeft]}>
          <View style={styles.tokenIcon}>
            {iconSource ? (
              ...
              <Text style={styles.tokenIconText}>{icon ?? symbol.slice(0, 1)}</Text>
            )}
          </View>
          <View>
...
export function AvatarCluster({ avatars }: { avatars: string[] }) {
  return (
    <View style={styles.cluster}>
      {avatars.slice(0, 3).map((avatar, idx) => (
        <View key={`${avatar}-${idx}`} style={[styles.clusterBubble, { marginLeft: idx === 0 ? 0 : -9 }]}> 
          <Text style={styles.clusterText}>{avatar}</Text>
        </View>
      ))}
      {avatars.length > 3 ? (
        <View style={[styles.clusterBubble, styles.clusterCount]}>
          <Text style={styles.clusterCountText}>{`${avatars.length - 3}+`}</Text>
        </View>
      ) : null}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  tradeCard: {
    ...
    borderRadius: 16,
    borderWidth: 1,
    borderColor: 'rgba(111, 118, 163, 0.2)',
  ...
  tokenIcon: {
    ...
    justifyContent: 'center',
    borderWidth: 1,
    borderColor: 'rgba(117, 122, 163, 0.22)',
  },
  tokenIconText: {
    color: theme.colors.textPrimary,
    fontSize: 20,
