import { Bip39, EnglishMnemonic, Random, Secp256k1, Slip10, Slip10Curve, ripemd160, sha256, stringToPath } from '@cosmjs/crypto';
...
function walletFromMnemonic(mnemonic: string) {
  return Bip39.mnemonicToSeed(new EnglishMnemonic(mnemonic)).then((seed) => {
    const { privkey } = Slip10.derivePath(Slip10Curve.Secp256k1, seed, stringToPath(bwickChain.hdPath));
    const keypair = Secp256k1.makeKeypair(privkey);
    const pubkey = Secp256k1.compressPubkey(keypair.pubkey);
    const address = toBech32(bwickChain.bech32Prefix, ripemd160(sha256(pubkey)));
