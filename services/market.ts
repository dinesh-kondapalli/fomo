...
async function fetchJson<T>(url: string): Promise<T> {
  ...
  for (let attempt = 0; attempt < RETRY_COUNT; attempt += 1) {
    ...
    if (!canRetry || attempt === RETRY_COUNT - 1) {
      throw lastError;
    }
...
function chartFromSparkline(values: number[], days: ChartRangeDays) {
  ...
  if (days === '1') {
    return takeLast(values, 48);
  }
  if (days === '7') {
    return values;
  }
  if (days === '30') {
    return takeLast(values, 120);
  }
...
export async function fetchLiveTokens(): Promise<LiveToken[]> {
  ...
  liveInFlight = (async () => {
    ...
    if (!tokens.length) {
      throw new Error('No market rows returned from CoinGecko');
    }
...
export async function fetchCoinDetails(coinId: string, days: ChartRangeDays = '1'): Promise<CoinDetails> {
  ...
  try {
    ...
    if (!market) {
      throw new Error(`No market data for ${normalizedId}`);
    }
    ...
    if (shouldLoadChartEndpoint) {
      try {
        ...
        if (endpointChart.length >= 2) {
          chart = endpointChart;
        }
      } catch {
        if (chart.length < 2) {
          chart = syntheticChart(market.current_price);
        }
      }
    }

    const oneHourChange = typeof market.price_change_percentage_1h_in_currency === 'number'
