...
export const bwickChain = {
  ...
  currencies: [
    { coinDenom: "BWICK", coinMinimalDenom: "ubwick", coinDecimals: 6 },
  ],
  feeCurrencies: [
    { coinDenom: "BWICK", coinMinimalDenom: "ubwick", coinDecimals: 6 },
  ],
...
type NodeInfoResponse = { default_node_info?: { network?: string } };
type BalancesResponse = { balances?: Array<{ denom: string; amount: string }> };

export async function getBwickChainStatus() {
  const url =
    typeof window === "undefined"
      ? `${bwickChain.rest}/cosmos/base/tendermint/v1beta1/node_info`
      : "/api/bwick/node-info";
...
export function isValidBwickAddress(address: string) {
  try {
    ...
    return (
      decoded.prefix === bwickChain.bech32Prefix && decoded.data.length === 20
    );
  } catch {
    return false;
  }
...
export async function getBwickBalance(address: string) {
  ...
  const url =
    typeof window === "undefined"
      ? `${bwickChain.rest}/cosmos/bank/v1beta1/balances/${address}`
      : `/api/bwick/balance/${encodeURIComponent(address)}`;
  const response = await fetch(url);
  ...
  const ubwick = data.balances?.find(
    (balance) => balance.denom === bwickChain.currencies[0].coinMinimalDenom,
  );
  return {
